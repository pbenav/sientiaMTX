...
class ProfileController extends Controller
{
    ...
    public function updateNotifications(Request $request): RedirectResponse
    {
        $validated = $request->validate([
            ...
            'notify_scheduled_before_minutes' => 'integer|min:0|max:1440',
        ...
        if ($request->input('tab') === 'notifications') {
            $newSettings = array_merge($current, [
                ...
                'notify_scheduled_tasks' => $request->boolean('notify_scheduled_tasks'),
                'notify_scheduled_before_minutes' => isset($validated['notify_scheduled_before_minutes']) ? (int) $validated['notify_scheduled_before_minutes'] : (int) ($current['notify_scheduled_before_minutes'] ?? 15),

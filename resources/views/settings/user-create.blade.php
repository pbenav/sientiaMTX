    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{{ __('Register a new user account manually.') }}</p>
            </div>
        </div>
    </x-slot>
            ...
            <form action="{{ route('settings.users.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select id="locale" name="locale" class="block mt-1 w-full border-gray-700 bg-gray-900 text-gray-300 focus:border-violet-500 focus:ring-violet-500 rounded-md shadow-sm">
                        <option value="es" {{ old('locale') === 'es' ? 'selected' : '' }}>{{ __('Spanish') }}</option>
                        <option value="en" {{ old('locale') === 'en' ? 'selected' : '' }}>{{ __('English') }}</option>
                    </select>
                ...
                <div class="flex items-center gap-4 mt-6">
                    ...
                    <a href="{{ route('settings.users') }}" class="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                        {{ __('Cancel') }}
                    </a>
                </div>
            </form>

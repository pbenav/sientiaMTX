...
<div x-data="telegramChat()" 
    ...
    <div x-show="open" 
        ...
        <!-- Cabecera -->
        <div @mousedown="startDrag($event)" @touchstart="startDrag($event)" class="px-6 py-4 bg-gradient-to-r from-sky-500 to-blue-600 flex items-center justify-between shadow-lg shrink-0 cursor-grab active:cursor-grabbing"
            ...
            <div class="relative flex items-center gap-2">
                ...
                <div class="relative flex-1">
                    ...
                    <textarea x-ref="chatInput" x-model="newMessage" 
                           ...
                           @keydown.arrow-down="if(showMentions) { $event.preventDefault(); mentionIndex = (mentionIndex + 1) % filteredMentions.length }"
                           @keydown.arrow-up="if(showMentions) { $event.preventDefault(); mentionIndex = (mentionIndex - 1 + filteredMentions.length) % filteredMentions.length }"
                           @keydown.enter="if(showMentions && filteredMentions.length > 0) { $event.preventDefault(); selectMention(filteredMentions[mentionIndex]); } else if(!$event.shiftKey) { $event.preventDefault(); send(); }"
                           @keydown.escape="if(showMentions) { $event.preventDefault(); showMentions = false; } else { $event.preventDefault(); cancelEdit(); }"

...
<div x-data="whatsappChat()" 
    ...
    <div x-show="open" 
        ...
        <div class="px-6 py-4 bg-gradient-to-r from-emerald-500 to-green-600 flex items-center justify-between shadow-lg shrink-0"
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h4 class="text-white font-bold text-sm leading-tight">WhatsApp MTX</h4>
        ...
        <!-- Área de Mensajes -->
        <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50 dark:bg-gray-950/50 custom-scrollbar" id="whatsapp-messages-container"
            ...
            <template x-for="msg in messages" :key="msg.id">
                <div :class="msg.from_me ? 'flex justify-end' : 'flex justify-start'" class="group relative">
                    <div :class="msg.from_me 
                        ? 'bg-green-600 text-white rounded-2xl rounded-tr-none px-4 py-2 shadow-md max-w-[85%]' 
                        ...
                        <div class="flex items-center justify-between gap-4 mt-1.5 border-t border-black/5 pt-1">
                            ...
                            <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <button @click="setReply(msg)"
                                        class="text-[9px] font-bold uppercase tracking-tighter"
                                        :class="msg.from_me ? 'text-white/70 hover:text-white' : 'text-green-500 hover:text-green-600'">
                                    {{ __('Responder') }}
                                </button>
            ...
            <div class="relative flex items-center gap-2">
                ...
                <div class="flex items-center gap-1">
                    <button type="button" @click="showEmojis = !showEmojis" :disabled="!teamId || isRecording"
                            class="p-2 text-gray-400 hover:text-emerald-500 dark:hover:text-emerald-400 transition-colors disabled:opacity-50"
                            title="Emojis">
                ...
                <div class="relative flex-1">
                    <textarea x-ref="chatInput" x-model="newMessage" 
                           ...
                           class="w-full rounded-2xl px-4 py-2 text-sm focus:ring-2 focus:ring-green-500 dark:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-inner resize-none min-h-[45px] max-h-[120px] custom-scrollbar"></textarea>
                    
                    <!-- Preview of pending reply -->
                    <template x-if="replyToId">
                        <div class="absolute bottom-full mb-3 left-0 right-0 bg-white dark:bg-gray-800 p-3 rounded-2xl shadow-2xl border-l-4 border-green-500 flex items-center justify-between gap-3 animate-in fade-in slide-in-from-bottom-2">
                ...
                <button type="submit" 
                        ...
                        :class="editingId ? 'bg-amber-600 hover:bg-amber-500 shadow-amber-500/30' : 'bg-green-600 hover:bg-green-500 shadow-green-500/30'"
    ...
    <button @mousedown="startDrag($event)" 
            ...
            class="group relative w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-emerald-400 to-green-600 backdrop-blur-sm rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 active:scale-95 ring-4 ring-white dark:ring-gray-950 pointer-events-auto"
...
<script>
    function whatsappChat() {
        ...
        return {
            ...
            initChat() {
                if (!this.teamId) {
                    this.messages = [{ id: 1, text: '⚠️ Entra en el panel de un equipo concreto para usar el chat de WhatsApp.', author: 'SientiaBot', from_me: false, time: 'Sistema' }];
            ...
            async refreshMessages(initial = false) {
                ...
                try {
                    const response = await fetch(`{{ route('whatsapp.chat.messages') }}?team_id=${this.teamId}`);
            ...
            async loadOlderMessages() {
                ...
                try {
                    const response = await fetch(`{{ route('whatsapp.chat.messages') }}?team_id=${this.teamId}&before_id=${this.firstMessageId}`);
                    const data = await response.json();
                ...
                for (let index in items) {
                    ...
                    if (item.kind === 'file') {
                        ...
                        if (blob && blob.type.startsWith('image/')) {
                            this.pendingPhoto = new File([blob], `whatsapp_paste_${new Date().getTime()}.png`, { type: blob.type });
            ...
            async updateMsg() {
                ...
                try {
                    const url = '{{ route("whatsapp.chat.update", ":id") }}'.replace(':id', id);
            ...
            async deleteMsg(msgId) {
                if (!confirm('¿Borrar mensaje? Se eliminará de WhatsApp y del historial.')) return;

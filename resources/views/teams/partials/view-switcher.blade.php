@php
    ...
    if (auth()->user()->can('view', $team)) {
        $views = [
            ...
            [
                'name' => 'Biblioteca',
                'route' => route('teams.library', $teamId),
                'active' => request()->routeIs('teams.library'),
    ...
    <div class="flex-1 min-w-0">
        <div class="flex items-center gap-0.5">
            @foreach ($views as $index => $view)
                ...
                @endif
                <a href="{{ $view['route'] }}"
                    class="flex flex-col items-center justify-center gap-0.5 px-1.5 md:px-2 py-2 rounded-xl transition-all shrink-0 min-w-max
                        {{ $view['active']
                    ...
                    title="{{ $view['name'] }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 lg:h-5 lg:w-5 shrink-0" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor" stroke-width="{{ $view['active'] ? '2.5' : '2' }}">
                        {!! $view['icon'] !!}
                    </svg>
                    <span class="hidden lg:block text-[9px] font-bold uppercase tracking-tight leading-none whitespace-nowrap">{{ $view['name'] }}</span>
                </a>
            @endforeach
            {{-- Divider + toggle --}}
            @if($isTaskList || $isKanban || $isGantt || $isMatrix)
                ...
                <div class="flex items-center gap-1 shrink-0 ml-1">
                    ...
                    <a href="{{ route('teams.trash.index', $team) }}" 
                       class="ml-2 flex items-center justify-center p-1.5 rounded-lg transition-colors bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 dark:hover:text-red-400 dark:text-red-400 shadow-sm"

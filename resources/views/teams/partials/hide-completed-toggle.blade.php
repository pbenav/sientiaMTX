...
@if($isVisibleView)
    <button onclick="toggleHideCompletedTasks()" 
        ...
        class="flex items-center gap-2 px-3 py-2 rounded-xl text-[10px] font-bold transition-all {{ $hideCompleted ? 'text-violet-600 dark:text-violet-400 bg-white dark:bg-gray-700 shadow-sm border border-gray-100 dark:border-gray-600' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300' }}">
        @if($hideCompleted)
        ...
        @endif
        <span class="hidden sm:inline">{{ $hideCompleted ? (__('tasks.completed_hidden') ?? 'Ocultas') : (__('tasks.completed_visible') ?? 'Visibles') }}</span>
    </button>

...
@if($isVisibleView)
    <button onclick="toggleHideCompletedTasks()" 
        ...
        <span class="hidden sm:block text-[9px] font-bold uppercase tracking-tight whitespace-nowrap leading-none">{{ $hideCompleted ? (__('tasks.completed_hidden') ?? 'Ocultas') : (__('tasks.completed_visible') ?? 'Visibles') }}</span>
    </button>

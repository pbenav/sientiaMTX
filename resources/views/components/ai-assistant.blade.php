        ...
        <div class="p-3 sm:p-4 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
            <form @submit.prevent="sendMessage" class="flex items-center space-x-2 sm:space-x-3">
                ...
                <div class="flex items-center gap-0.5 sm:gap-1 shrink-0">
                    ...
                    <input type="file" x-ref="fileInput" @change="handleFileUpload" class="hidden" accept="audio/*,image/*,.pdf,application/pdf">
                </div>
                <button 
                    type="submit" 
    ...
    (function() {
        const registerAssistant = () => {
                ...
                try {
                    ...
                    <div class="group/payload my-6 relative transition-all duration-500">
                        ...
                        <div class="relative p-6 rounded-[2.5rem] bg-indigo-50/90 dark:bg-slate-900 border border-white/50 dark:border-slate-800 shadow-2xl backdrop-blur-xl">
                            ...
                            <div class="prose prose-sm dark:prose-invert max-w-none text-slate-700 dark:text-slate-300 text-[13px] leading-relaxed font-medium">${typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(marked.parse(payloadContent)) : marked.parse(payloadContent).replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '').replace(/on[a-z]+=/gi, '')}</div>
            ...
            renderPayloadPreview(content) {
                    ...
                    try {
                        ...
                        if (obj.title || obj.name) {
                            ...
                            return `<div class="flex flex-col gap-1">
                                ...
                                <div class="mt-2 text-sm text-gray-600 dark:text-gray-400">${highlightPatterns(typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(marked.parse(desc)) : marked.parse(desc).replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '').replace(/on[a-z]+=/gi, ''))}...</div>
                            </div>`;
                        }
                ...
                try {
                    return highlightPatterns(typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(marked.parse(content)) : marked.parse(content).replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '').replace(/on[a-z]+=/gi, ''));
                } catch (e) {
                ...
                if (isDirect) {
                    ...
                    if (targetEl) {
                        ...
                        if (isQuill && actualEditor) {
                            actualEditor.innerHTML += typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(marked.parse(textToInject)) : marked.parse(textToInject).replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '').replace(/on[a-z]+=/gi, '');
                ...
                await Swal.fire({
                    ...
                    html: `
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 p-2">
                            ...
                            <!-- Decisión -->
                            <button type="button" onclick="window._aiSelectType('agreement')" class="flex flex-col items-center gap-2 p-4 rounded-[1.5rem] border-2 border-rose-100 dark:border-rose-900/30 bg-white dark:bg-slate-900 hover:border-rose-600 transition-all text-center group">

            ...
            <div class="w-full mt-6 mb-4">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between bg-gray-100/50 dark:bg-gray-800/50 p-1.5 rounded-2xl border border-gray-200/50 dark:border-gray-700/50 shadow-sm gap-3">
                    <div class="flex items-center gap-0.5 overflow-x-auto no-scrollbar">
                        {{-- Escritorio --}}
                        <a href="{{ $team ? route('teams.time-reports', $team) : route('teams.index') }}"
                        ...
                        <a href="{{ route($routePrefix . 'show', $team ? [$team, $survey] : [$survey]) }}"
                            ...
                            <div class="flex justify-between items-start mb-3">
                                ...
                                <div
                                    ...
                                    @can('delete', $survey)
                                        <object>
                                            <form action="{{ route($routePrefix . 'destroy', $team ? [$team, $survey] : [$survey]) }}" method="POST" class="inline-block m-0 p-0" onsubmit="event.stopPropagation(); return confirm('{{ __('¿Estás seguro de que quieres eliminar esta encuesta?') }}')">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="p-1 text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors" onclick="event.stopPropagation()" title="Eliminar Encuesta">

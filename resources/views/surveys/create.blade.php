<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('teams.surveys.index', $team) }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                {{ __('Cancelar') }}
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('teams.surveys.store', $team) }}" method="POST" id="surveyForm">
                        @csrf
                        ...
                        <div class="mb-6">
                            ...
                            <select name="type" id="type" required
                                ...
                                <option value="single_choice" {{ old('type') === 'single_choice' ? 'selected' : '' }}>{{ __('Opción única') }}</option>
                                <option value="multiple_choice" {{ old('type') === 'multiple_choice' ? 'selected' : '' }}>{{ __('Opción múltiple') }}</option>
                                <option value="rating" {{ old('type') === 'rating' ? 'selected' : '' }}>{{ __('Valoración (1-5)') }}</option>
                                <option value="text" {{ old('type') === 'text' ? 'selected' : '' }}>{{ __('Texto libre') }}</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div id="optionsContainer" class="mb-6 {{ old('type') !== 'single_choice' && old('type') !== 'multiple_choice' ? 'hidden' : '' }}">
                            ...
                            <div id="optionsList" class="space-y-3">
                                ...
                                @else
                                    <div class="flex items-center gap-2 option-item">
                                        <input type="color" name="option_colors[]" value="#3B82F6" class="w-10 h-8 rounded cursor-pointer" />
                                    ...
                                    <div class="flex items-center gap-2 option-item">
                                        <input type="color" name="option_colors[]" value="#EF4444" class="w-10 h-8 rounded cursor-pointer" />
                        ...
                        <!-- Texto libre (para tipo text) -->
                        <div id="textContainer" class="mb-6 {{ old('type') !== 'text' ? 'hidden' : '' }}">
                        ...
                        <!-- Valoración (para tipo rating) -->
                        <div id="ratingContainer" class="mb-6 {{ old('type') !== 'rating' ? 'hidden' : '' }}">
                        ...
                        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                            ...
                            <div class="space-y-3">
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="allow_multiple_votes" value="1" {{ old('allow_multiple_votes') ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    <span class="ml-2 text-sm text-gray-700">{{ __('Permitir múltiples votos') }}</span>
                                </label>

                                <label class="flex items-center">
                                    <input type="checkbox" name="show_results_before_voting" value="1" {{ old('show_results_before_voting') ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                <div>
                                    ...
                                    <input type="datetime-local" name="expires_at" value="{{ old('expires_at') ? now()->addDays(7)->format('Y-m-d\TH:i') : '' }}"
